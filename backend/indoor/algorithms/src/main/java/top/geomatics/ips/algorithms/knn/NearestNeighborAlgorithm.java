...
import top.geomatics.ips.algorithms.base.InsertionSort;
import top.geomatics.ips.algorithms.base.QuickSort;
...
public class NearestNeighborAlgorithm implements PositioningAlgorithm {
	...
	public PositioningResult calculatePosition(WLANMeasurement measurements) {
		...
		/**
		 * Calculate the distances from this measurement to the different figerprints
		 */
		Map<WLANFingerprint, DistanceResult> dists = calculateDistances(fingerprints, measurements);
	 ...
	 * 
	 * @param fps The sample fingerprints
	 * @param m   The new measurement
	 * @return A map containing the different fingerprints and their corresponding
	 *         distance result
	 */
	private Map<WLANFingerprint, DistanceResult> calculateDistances(WLANFingerprint[] fps, WLANMeasurement m) {
	 ...
	 * Distanceresult> Map.
	 * 
	 * First does a quicksort on the matched number of distances. A limited number
	 * of values is taken from this and sorted again, now using an insertion sort,
	 * on there distances.
	 * 
	 * @param dists
	 * @return
	...
	private Entry<WLANFingerprint, DistanceResult> findSmallestDistance(Map<WLANFingerprint, DistanceResult> dists) {

		/**
		 * Change the map into an array so that it can be sorted by the QuickSort
		 * algorithm.
		 */
		...
		entries = new InsertionSort().sort((Entry<WLANFingerprint, DistanceResult>[]) partialSet);

		/**
		 * The final result is stored in a special NNResults object which contains an
		 * list of all the DistanceResults
		 */
	 ...
	 * 
	 * The result is stored in a map that has as keys the fingerprint to which the
	 * distance is calculated and as value the {@link DistanceResult}.
	 * 
	 * @author Wouter Van Rossem
	 * 
	 */
	class EuclDistTask extends RecursiveTask<Map<WLANFingerprint, DistanceResult>> {
		...
		public EuclDistTask(WLANFingerprint[] fps, WLANMeasurement mm, int low, int high) {
		...
		/**
		 * Computes the distance between the measurement and an array of fingerprints by
		 * calculating it immediately if the sequential cutoff point is reached or
		 * forking two new threads and waiting to join the results.
		 */
		@Override
		protected Map<WLANFingerprint, DistanceResult> compute() {
			...
			if (high - low < SEQUENTIAL_CUTOFF) {
				 ...
				 * 
				 * Loop over the fingerprints and calculate the distance from the measurement to
				 * itself.
				 */
				...
				/**
				 * The array is still large enough an can so the task can be further forked to
				 * two separate tasks.
				 * 
		...
		/**
		 * Calculates the Euclidian distance between a measurement and a fingerprint.
		 * 
		 * @param mm The measurement
		 * @param fp The fingerprint to calculate the distance
		 * @return
		 */
		private DistanceResult calcEuclDist(WLANMeasurement mm, WLANFingerprint fp) {

			double d = 0;

			/**
			 * We keep track of exactly how many access points with the same BSSID occur in
			 * the measurement and the fingerprint. Since a higher amount of equal access
			 * points will lead to a better result.
			 */
			int nrOfMatchedBssids = 0;

			/**
			 * Loop over each access point from the measurement, get the access point
			 * average power interval of a access point with the same BSSID (if it exists).
			 * Subtract them and square that result.
			 */
			for (Entry<AccessPoint, Integer> m : mm.getLevels().entrySet()) {

				// Get an access point from the fingerprint with a same BSSID
				AccessPointPowerLevels levels = fp.getApLevels(m.getKey().getBSSID());
			...
			/**
			 * Return the euclidian distance by taking the square root of d. We also return
			 * the number of matched access points and the figerprint to which the distance
			 * was calculated in a DistanceResult object.
			 */
