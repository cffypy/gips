...
//    步长
    ...
    public void onSensorChanged(SensorEvent event) {  //这个接口实现的方法onSensorChanged(SensorEvent event),会返回传感器回调的数值，传入calc_step(event)方法
    ...
    synchronized private void  calc_step(SensorEvent event) {   //calc_step方法算出加速度传感器的x、y、z三轴的平均数值（为了平衡在某一个方向数值过大造成的数据误差），接着交给DetectorNewStep方法处理。
...
//        计算步长采用公式stepLength=K*(max_a-min_a)^0.25
        if(peakOfWave!=0&&valleyOfWave!=0){
            stepLength = 0.4153 * Math.pow(peakOfWave - valleyOfWave, 0.25);
        }
        stepCallBack.Step(StepSensorBase.CURRENT_SETP,stepLength);
//            }
//        }
...
//        Log.v(TAG, "oldValue:" + oldValue);
        if (!isDirectionUp && lastStatus
            ...
            peakOfWave = oldValue;
            return true;
        } else if (!lastStatus && isDirectionUp && oldValue<max_valley) {
